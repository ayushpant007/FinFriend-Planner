...
async function loadBenchmarkData(benchmarkFile: string): Promise<RawDataPoint[]> {
  const filePath = path.join(process.cwd(), 'public', 'Fund Benchmark past 10 years', benchmarkFile);
...
export async function POST(request: Request) {
  try {
    ...
    console.log('[EquityPortfolio] Received funds:', JSON.stringify(funds, null, 2));
    
    if (funds.length === 0) {
      console.log('[EquityPortfolio] No funds provided');
    ...
    console.log('[EquityPortfolio] NAV data counts:', allFundNavs.map((nav, i) => `${funds[i].schemeName}: ${nav.length} points`));
    ...
    for (let i = 0; i < funds.length; i++) {
      ...
      if (navData.length < 12) {
        console.warn(`[EquityPortfolio] Fund ${fund.schemeName} has insufficient NAV history: ${navData.length} points`);
        continue;
      }
      
      const benchmarkFile = getBenchmarkFileForFund(fund.schemeName);
      console.log(`[EquityPortfolio] Fund "${fund.schemeName}" -> Benchmark: ${benchmarkFile}`);
      
      if (!benchmarkCache[benchmarkFile]) {
        benchmarkCache[benchmarkFile] = await loadBenchmarkData(benchmarkFile);
        console.log(`[EquityPortfolio] Loaded benchmark ${benchmarkFile}: ${benchmarkCache[benchmarkFile].length} points`);
      }
      
      const benchmarkData = benchmarkCache[benchmarkFile];
      if (benchmarkData.length === 0) {
        console.warn(`[EquityPortfolio] No benchmark data for ${fund.schemeName}`);
        continue;
      }
    ...
    console.log(`[EquityPortfolio] Valid funds after NAV/benchmark check: ${validFundsRaw.length}`);
    
    if (validFundsRaw.length === 0) {
      console.log('[EquityPortfolio] No valid funds - returning empty chartData');
      return NextResponse.json({ chartData: [] });
    }
    ...
    console.log('[EquityPortfolio] NAV start dates:', allNavStartDates.map(d => d.toISOString().split('T')[0]));
    console.log('[EquityPortfolio] NAV end dates:', allNavEndDates.map(d => d.toISOString().split('T')[0]));
    console.log('[EquityPortfolio] Benchmark start dates:', allBenchmarkStartDates.map(d => d.toISOString().split('T')[0]));
    console.log('[EquityPortfolio] Benchmark end dates:', allBenchmarkEndDates.map(d => d.toISOString().split('T')[0]));
    ...
    console.log(`[EquityPortfolio] Common date range: ${commonStartDate.toISOString().split('T')[0]} to ${commonEndDate.toISOString().split('T')[0]}`);
    
    if (commonStartDate >= commonEndDate) {
      console.log('[EquityPortfolio] Invalid date range - returning empty chartData');
      return NextResponse.json({ chartData: [] });
    }
    ...
    if (validFunds.length === 0) {
      console.log('[EquityPortfolio] No valid funds after base value check - returning empty chartData');
      return NextResponse.json({ chartData: [] });
    }
    
    console.log(`[EquityPortfolio] Valid funds for chart: ${validFunds.length}`);
    ...
    console.error('Error generating equity portfolio growth data:', error);
    return NextResponse.json({ chartData: [], error: 'Failed to generate portfolio data' }, { status: 500 });
  }
}
