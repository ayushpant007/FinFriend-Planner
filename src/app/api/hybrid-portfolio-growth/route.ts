...
import { getHybridBenchmarkFileForFund } from '@/lib/hybrid-benchmark-mapping';
...
async function loadHybridBenchmarkData(benchmarkFile: string): Promise<RawDataPoint[]> {
  const filePath = path.join(process.cwd(), 'public', 'Fund Benchmark (hybrid)', benchmarkFile);
  
  if (!fs.existsSync(filePath)) {
    console.error(`Hybrid benchmark file not found: ${filePath}`);
    return [];
  }
...
export async function POST(request: Request) {
  try {
    ...
    console.log('[HybridPortfolio] Received funds:', JSON.stringify(funds, null, 2));
    
    if (funds.length === 0) {
      console.log('[HybridPortfolio] No funds provided');
    ...
    console.log('[HybridPortfolio] NAV data counts:', allFundNavs.map((nav, i) => `${funds[i].schemeName}: ${nav.length} points`));
    ...
    for (let i = 0; i < funds.length; i++) {
      ...
      if (navData.length < 12) {
        console.warn(`[HybridPortfolio] Fund ${fund.schemeName} has insufficient NAV history: ${navData.length} points`);
        continue;
      }
      
      const benchmarkFile = getHybridBenchmarkFileForFund(fund.schemeName);
      console.log(`[HybridPortfolio] Fund "${fund.schemeName}" -> Benchmark: ${benchmarkFile}`);
      
      if (!benchmarkCache[benchmarkFile]) {
        benchmarkCache[benchmarkFile] = await loadHybridBenchmarkData(benchmarkFile);
        console.log(`[HybridPortfolio] Loaded benchmark ${benchmarkFile}: ${benchmarkCache[benchmarkFile].length} points`);
      }
      
      const benchmarkData = benchmarkCache[benchmarkFile];
      if (benchmarkData.length === 0) {
        console.warn(`[HybridPortfolio] No benchmark data for ${fund.schemeName}`);
        continue;
      }
    ...
    console.log(`[HybridPortfolio] Valid funds after NAV/benchmark check: ${validFundsRaw.length}`);
    
    if (validFundsRaw.length === 0) {
      console.log('[HybridPortfolio] No valid funds - returning empty chartData');
      return NextResponse.json({ chartData: [] });
    }
    ...
    console.log('[HybridPortfolio] NAV start dates:', allNavStartDates.map(d => d.toISOString().split('T')[0]));
    console.log('[HybridPortfolio] NAV end dates:', allNavEndDates.map(d => d.toISOString().split('T')[0]));
    console.log('[HybridPortfolio] Benchmark start dates:', allBenchmarkStartDates.map(d => d.toISOString().split('T')[0]));
    console.log('[HybridPortfolio] Benchmark end dates:', allBenchmarkEndDates.map(d => d.toISOString().split('T')[0]));
    ...
    console.log(`[HybridPortfolio] Common date range: ${commonStartDate.toISOString().split('T')[0]} to ${commonEndDate.toISOString().split('T')[0]}`);
    
    if (commonStartDate >= commonEndDate) {
      console.log('[HybridPortfolio] Invalid date range - returning empty chartData');
      return NextResponse.json({ chartData: [] });
    }
    ...
    if (validFunds.length === 0) {
      console.log('[HybridPortfolio] No valid funds after base value check - returning empty chartData');
      return NextResponse.json({ chartData: [] });
    }
    
    console.log(`[HybridPortfolio] Valid funds for chart: ${validFunds.length}`);
    ...
    console.error('[HybridPortfolio] Error:', error);
    return NextResponse.json({ error: 'Failed to generate hybrid portfolio chart' }, { status: 500 });
  }
}
